 ...
 *
 * This is a cpu light baking Plugin for Three.js Version r71
 * With this Plugin we provide two baking algorithms.
 * With this plugin it's possible to achieve some gi-effects such as colorbleeding, indirect lightning and hard and softshadows.
 *
...
//------------------------------------//
// PLUGIN DEVELOPMENT:
// Better following the prototype or the closure method?
// it seems that a closure should be better:
// http://www.samselikoff.com/blog/some-Javascript-constructor-patterns/
// "... if the object you're making is large and complex, and is designed mainly for public consumption, the flexibility
// that comes with having private properties and methods will probably win out, and you'll want to take advantage of
// closures. This is why libraries like jQuery and D3 tend to use closures: users will almost certainly only be using
// one instance of the library, which means that method sharing becomes less of an issue."
//------------------------------------//
...
( function ( THREE ) {
        ...
        THREE.cLightBaking = {
            ...
            // Illumination models
            IlluminationModelEnum: {
                LAMBERT: 0
            },

            // Run the Application in: TODO
            ApplicationExecutionEnum: {
        ...
        THREE.LightBaking = function ( parameters ) {
            ...
            if ( _config.appMode === THREE.cLightBaking.ApplicationExecutionEnum.MULTITHREADED && typeof(Worker) === "undefined" ) {
            ...
            if ( _config.raycasterImplementation === THREE.cLightBaking.RayCasterEnum.OCTREE && typeof(THREE.Octree) === "undefined" ) {
            ...
            if ( _config.uvMethod === THREE.cLightBaking.UVMethodEnum.PACKED && typeof(GrowingPacker) === "undefined" ) {
            ...
            if ( _config.bakingMethod === THREE.cLightBaking.BakingMethodEnum.TWOPASS ) {

                if ( _config.appMode === THREE.cLightBaking.ApplicationExecutionEnum.MULTITHREADED ) {
            ...
            if ( _config.bakingMethod === THREE.cLightBaking.BakingMethodEnum.PATHTRACING ) {
...
//-----------------------------------------------//
// ADD SOME FUNCTIONS TO THREE METHODS
//-----------------------------------------------//
...
// From THREE.Vector3
        ...
        function getDefaultConfig() {
            ...
            return {
                ...
                // Shading Technique
                shading: THREE.cLightBaking.ShadingEnum.PHONG,

                // Illumination Model
                illuminationModel: THREE.cLightBaking.IlluminationModelEnum.LAMBERT,

                // UV
                uvMethod: THREE.cLightBaking.UVMethodEnum.PACKED, // 0 - first try, 1 - simple centered, 2 - bin packing approach
                packingOffset: 2,
                uvSmoothing: 0.2,
                ...
                // various
                specificMeshBaking: THREE.cLightBaking.SpecificMeshBakingEnum.DISABLED,
                specificRayCasting: THREE.cLightBaking.SpecificRayCastingEnum.DISABLED,

                raycasterImplementation: THREE.cLightBaking.RayCasterEnum.THREEJS,
                ...
                workerId: -1,
                workerTaskId: 0,
                workerTaskMode: THREE.cLightBaking.WorkerTaskEnum.MESH,
        ...
        var cachedWorkerThreadPool = (function () {
            ...
            function onMessage( event ) {
                ...
                if ( event.data.intent === THREE.cLightBaking.WorkerTaskEnum.FINISHED ) {
        ...
        var raycasterFunCtor = function ( implementation ) {
            ...
            function ctor() {
                switch ( implementation ) {
                    case THREE.cLightBaking.RayCasterEnum.THREEJS:


                        break;

                    case THREE.cLightBaking.RayCasterEnum.OCTREE:
        ...
        var uvFunCtor = function ( mode ) {
            ...
            function calcYOffset() {
                ...
                return (function () {
                    ...
                    switch ( mode ) {
                        case THREE.cLightBaking.UVMethodEnum.UNIFORMUNCENTERED:

                            return def;

                        case THREE.cLightBaking.UVMethodEnum.UNIFORMCENTERED:
                        case THREE.cLightBaking.UVMethodEnum.PACKED:
        ...
        var bakeMesh = function ( ppass, pml ) {
            ...
            return function ( mesh, m ) { // return from bakeMesh
                ...
                if ( _config.appMode === THREE.cLightBaking.ApplicationExecutionEnum.ASYNC ) {
            ...
            __scene.traverse( function ( mesh ) {
                    ...
                    if ( mesh instanceof THREE.Light ) {
                        ...
                        switch ( _config.specificMeshBaking ) {
                            ...
                            case THREE.cLightBaking.SpecificMeshBakingEnum.DISABLED:

                                mesh.userData.baking.bakeMe = true;
                                break;

                            case THREE.cLightBaking.SpecificMeshBakingEnum.ENABLED:
                            ...
                            case THREE.cLightBaking.SpecificMeshBakingEnum.INVERTED:
                        ...
                        if ( isReadyToCreateFaceVertexLightMapUv( mesh ) ) {
                            ...
                            if ( (_config.appMode !== THREE.cLightBaking.ApplicationExecutionEnum.MULTITHREADED) && (mesh.userData.baking.bakeMe || (_config.bakingMethod === THREE.cLightBaking.BakingMethodEnum.PATHTRACING && _config.shading === THREE.cLightBaking.ShadingEnum.PHONG)) ) {
                        ...
                        if ( mesh.userData.baking.bakeMe && mesh.material instanceof THREE.MeshBasicMaterial || mesh.material instanceof THREE.MeshPhongMaterial || mesh.material instanceof THREE.MeshLambertMaterial ) {
                            ...
                            if ( mesh.userData.baking.faceEndOffset === undefined ) {
                                ...
                                if ( _config.workerId > -1 ) {
                                    ...
                                    if ( _config.workerTaskMode === THREE.cLightBaking.WorkerTaskEnum.MESH && mesh.userData.baking.faceEndOffset === -1 && mesh.userData.baking.bakeMe ) {
                        ...
                        switch ( _config.specificRayCasting ) {
                            ...
                            case THREE.cLightBaking.SpecificRayCastingEnum.DISABLED:

                                mesh.userData.baking.intersectMe = true;
                                break;

                            case THREE.cLightBaking.SpecificRayCastingEnum.ENABLED:
                            ...
                            case THREE.cLightBaking.SpecificRayCastingEnum.INVERTED:
                ...
                for ( ty = 1 ; ty < m.userData.baking.textureHeight - 2 ; ty++ ) {
                    ...
                    for ( tx = 1 ; tx < m.userData.baking.textureWidth - 2 ; tx++ ) {
                        ...
                        switch ( _config.postProcessingFilter ) {
                            case THREE.cLightBaking.FilterEnum.BOX:
                                r = (r / factor) | 0;
                                g = (g / factor) | 0;
                                b = (b / factor) | 0;
                                break;
                            case THREE.cLightBaking.FilterEnum.GAUSS:
                                break;
                        }
        ...
        function run( afterBakingCallBack ) {
            ...
            bakingSetup( function () {
                ...
                switch ( _config.appMode ) {
                    case THREE.cLightBaking.ApplicationExecutionEnum.SINGLETHREADED:
                    case THREE.cLightBaking.ApplicationExecutionEnum.ASYNC:
                    ...
                    case THREE.cLightBaking.ApplicationExecutionEnum.MULTITHREADED:


                        log( "rayTracePreparation() - worker Mode" );

                        // do not initiate unecessary workers
                        if ( _config.workerTaskMode === THREE.cLightBaking.WorkerTaskEnum.MESH && (__sceneObjectsToBake.length < workerLimit) ) {
                        ...
                        switch ( _config.workerTaskMode ) {
                            ...
                            default:
                                ...
                                cachedWorkerThreadPool.setOnTaskMessage( function ( evt ) {
                                        ...
                                        if ( evt.data.intent === THREE.cLightBaking.WorkerTaskEnum.MESH ) {
                            ...
                            switch ( _config.workerTaskMode ) {
                                ...
                                default :
                                    ...
                                    cachedWorkerThreadPool.appendTask( {
                                        intent: THREE.cLightBaking.WorkerTaskEnum.MESH,
                                        uuid: mesh.uuid
                                    } );
                                ...
                                case THREE.cLightBaking.WorkerTaskEnum.FACE:
                                    ...
                                    for ( faceIndex = mesh.userData.baking.faceBeginOffset, faceEnd = mesh.userData.baking.faceEndOffset ; faceIndex < faceEnd ; faceIndex++ ) {
                                        ...
                                        cachedWorkerThreadPool.appendTask( {
                                            intent: THREE.cLightBaking.WorkerTaskEnum.FACE,
                                            uuid: mesh.uuid,
                                            faceIndex: faceIndex
                                        } );
        ...
        function executed() {
            ...
            if ( _config.postProcessingFilter !== THREE.cLightBaking.FilterEnum.NONE && _config.appMode !== THREE.cLightBaking.ApplicationExecutionEnum.MULTITHREADED ) {
        ...
        function lightMapGenerationPerLumel( verts, faces, faceVertexLightMapUv, matrixWorld, scene, pass, material, tw, th, meshuuid, texBuf, faceBeginOffset, faceEndOffset, uvInfo, raycaster ) {
            ...
            for ( faceIndex = faceBeginOffset ; faceIndex < faceEndOffset ; faceIndex++ ) {
                ...
                if ( _config.shading === THREE.cLightBaking.ShadingEnum.FLATFAST ) {
                ...
                for ( y = uvOffsets.yBegin ; y < uvOffsets.yEnd ; y++ ) {
                    ...
                    for ( x = uvOffsets.xBegin ; x < uvOffsets.xEnd ; x++ ) {
                        ...
                        switch ( _config.bakingMethod ) {
                            ...
                            case THREE.cLightBaking.BakingMethodEnum.TWOPASS:
                            ...
                            case THREE.cLightBaking.BakingMethodEnum.PATHTRACING:
            ...
            return (_config.shading === THREE.cLightBaking.ShadingEnum.FLATFAST) ? colorizeFaceFun.getImageData() : buf;
        }
        ...
        function rendererCalculateLumelColor( bary, origin, rayDir, raycaster, verts, face, matrixWorld, scene, numLevels, _ci ) {
            ...
            if ( _config.shading === THREE.cLightBaking.ShadingEnum.PHONG ) {
            ...
            if ( intensity > 0 ) {
                ...
                if ( isVisible ) {
                    ...
                    switch ( _config.shading ) {
                        ...
                        case THREE.cLightBaking.ShadingEnum.FLAT:
                        case THREE.cLightBaking.ShadingEnum.FLAT:
                        ...
                        case THREE.cLightBaking.ShadingEnum.PHONG:
                    ...
                    switch ( _config.illuminationModel ) {
                        ...
                        case THREE.cLightBaking.IlluminationModelEnum.LAMBERT:
